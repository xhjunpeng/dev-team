...
def validate_migration(value: object) -> None:
    ...
    if not isinstance(source_version, int) or source_version < 1 or source_version >= CURRENT_PROTOCOL_VERSION:
...
def migrate_v1_state(state: dict) -> dict:
    ...
    legacy["protocol_version"] = CURRENT_PROTOCOL_VERSION
    legacy["migration"] = {
...
def validate_scope(scope: dict, business_goal: str, repo: Path, state_path: Path, baseline: str, check_diff: bool) -> None:
...
def validate(state: dict, repo: Path, state_path: Path, state_dir: Path, check_diff: bool, check_worktree: bool, check_location: bool) -> None:
    required = {"protocol_version", "migration", "task_id", "lifecycle", "skill_root", "git_baseline", "ignored_untracked_policy", "primary_branch", "authorization_card", "candidate", "failure_identity", "production_failure_count", "failure_records", "recovery", "write_scope"}
    if set(state) != required:
        fail("STATE_FIELDS_INVALID")
    if state["protocol_version"] != CURRENT_PROTOCOL_VERSION:
        fail("PROTOCOL_VERSION_INVALID")
    validate_migration(state["migration"])
    ...
    validate_scope(require_object(state["write_scope"], "WRITE_SCOPE"), card["business_goal"], repo, state_path, baseline, check_diff)
